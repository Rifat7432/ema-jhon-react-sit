  ...
  return (
    <div className="reletive">
      <div className="bg-red-50 h-5/6 text-slate-900 cart">
        ...
        <div>
          <button onClick={clear} className="btn btn-error block mt-5 w-full">
            Clear Cart <FontAwesomeIcon icon={faTrashAlt} />
          </button>
          <Link to="/orders">
            <button className="btn btn-warning block mt-5 w-full">
              Review Orders <FontAwesomeIcon icon={faArrowRight} />
            </button>
          </Link>
        </div>
      </div>
